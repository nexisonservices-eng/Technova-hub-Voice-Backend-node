    ...
    async getWorkflowEventLog(workflowId, startDate, endDate, limit = 250, userId = null) {
        try {
            ...
            const rows = executions.map((execution) => {
                ...
                const whatsappState = booking
                    ? {
                        ...
                        customerProviderMessageId: customerNotification?.providerMessageId || '',
                        adminProviderMessageId: adminNotification?.providerMessageId || '',
                ...
                return {
                    ...
                    customerProviderMessageId: whatsappState?.customerProviderMessageId || '',
                    adminProviderMessageId: whatsappState?.adminProviderMessageId || '',
            ...
            const summary = {
                ...
                whatsappSent: rows.filter((row) => ['sent', 'delivered', 'read'].includes(row.customerWhatsAppStatus) || ['sent', 'delivered', 'read'].includes(row.adminWhatsAppStatus)).length,
